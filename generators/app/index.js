...
module.exports = class extends Generator {
    ...
    prompting() {
        ...
        return this.prompt(promptInit)
            ...
            .then((initProps) => {
                ...
                if (templateName === this.rootJSON.tplPromptInit.custom) {
                    // 自定义选项询问
                    const customPrompts = [
                        {
                            type: 'input',
                    ...
                    return this.prompt(customPrompts.concat(prompts)).then(props => {
    ...
    writing() {
        if (this.choiceTemplateName === this.rootJSON.tplPromptInit.default) {
            this._copy();
        } else {
    ...
    _downloadTemplate() {
        ...
        const me = this;
        const done = this.async();
        // console.log("_downloadTemplate!!!222")
        const template = templateConfig.filter(item => item.name === this.choiceTemplateName)[0];
        const templateUrl = this.choiceTemplateName === this.rootJSON.tplPromptInit.custom ? this.customUrl : template.url;
        ...
        download(`direct:${templateUrl}`, dirPath, { clone: true }, err => {
            if (err) {
                // console.log(err)
                // console.log(`direct:${templateUrl}`)
                // console.log(dirPath)
